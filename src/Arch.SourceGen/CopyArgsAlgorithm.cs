...
/// <summary>
///     <see cref="CopyArgsAlgorithm"/> repeats a given argument of structure <c>{name}__{variadicType}</c>, separated by commas. It properly copies any ref, in, out, etc modifiers.
...
///     <code>
///         MyMethod&lt;T0&gt;(in component__T0)
///     </code>
///     ... will convert to:
///     <code>
///         MyMethod&lt;T0, T1&gt;(in component__T0, in component__T1)
///     </code>
/// </example>
[VariadicAlgorithm]
internal class CopyArgsAlgorithm : LineAlgorithm
{
    ...
    public override string Transform(string line, string type, int start, int variations, string[] parameters)
    {
        ...
        // match ref, in, out
        var modifiersMatch = Regex.Match(line, $@"[(,]\s*(?<Modifiers>(ref|out ref|out var|out {type}\??|in|ref {type}\??))?\s*{parameters[0]}__{type}");
        if (!modifiersMatch.Success)
        {
            throw new InvalidOperationException($"Can't find variable {parameters[0]}__{type} in a parameter list.");
        }

        var modifiers = modifiersMatch.Groups["Modifiers"].Value;

        newVariables.Append($"{modifiers} {parameters[0]}__{type}");
        for (int i = start; i < variations; i++)
        {
            var variadic = VaryType(type, i);
            newVariables.Append($", {modifiers} {parameters[0]}__{variadic}");
        }
